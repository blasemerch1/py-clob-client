        ...
        try:
            ...
            if isinstance(price_data, str):
                try:
                    ...
                    logger.warning(f"Received invalid JSON for price history of market {market_hash}")
                    return []
            ...
            # Format the price history data, ensuring we're dealing with dictionary objects
            ...
            for entry in price_data:
                if isinstance(entry, str):
                    try:
                        ...
                        logger.warning(f"Skipping invalid price history entry: {entry}")
                        continue
                
                if not isinstance(entry, dict):
                    logger.warning(f"Skipping non-dictionary price history entry: {entry}")
                    continue
        ...
        try:
            ...
            if isinstance(trade_data, str):
                try:
                    ...
                    logger.warning(f"Received invalid JSON for trade history of market {market_hash}")
                    return []
            ...
            # Format the trade history data, ensuring we're dealing with dictionary objects
            ...
            for entry in trade_data:
                if isinstance(entry, str):
                    try:
                        ...
                        logger.warning(f"Skipping invalid trade history entry: {entry}")
                        continue
                
                if not isinstance(entry, dict):
                    logger.warning(f"Skipping non-dictionary trade history entry: {entry}")
                    continue
...
def fetch_market_data(client: ClobClient) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Ensure the response is a valid dictionary/list
        if isinstance(markets_response, str):
            try:
                markets = json.loads(markets_response)
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse markets response as JSON: {str(e)}")
                raise Exception(f"Invalid JSON response from get_markets(): {str(e)}")
        else:
            markets = markets_response
        
        if not isinstance(markets, list):
            logger.error(f"Expected list from get_markets(), got {type(markets)}")
            raise Exception(f"Invalid response format from get_markets(): {markets}")
        ...
        for market in markets:
            ...
            if isinstance(market, str):
                try:
                    ...
                    logger.warning(f"Skipping market with invalid JSON: {market}")
                    continue
            
            if not isinstance(market, dict):
                logger.warning(f"Skipping non-dictionary market entry: {market}")
                continue
